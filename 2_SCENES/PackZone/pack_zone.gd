...
func _on_ui_expansion_selected(ExpansionID: DATA.ExpansionIDs, SinglePack: bool) -> void:
	var value_in_packs   : int   = 1 if SinglePack else 10
	...
	if SinglePack and COLLECTION.next_pack_timestamp < Time.get_unix_time_from_system(): 
		pull_single_pack(ExpansionID)
	elif needed_charges <= COLLECTION.timer_charges:
		...
		popup.finished.connect(func(rc: PopUpConfirm.ReturnCodes):
			if rc == PopUpConfirm.ReturnCodes.CONFIRM:
				COLLECTION._spend_timer_charges(needed_charges)
				if SinglePack: pull_single_pack(ExpansionID) 
				else: pull_booster_box(ExpansionID)
		)
...
func pull_booster_box(ExpansionID: DATA.ExpansionIDs):
	...
	while counts[DATA.ContentTypes.CRITTER]    < 5 or \
		  counts[DATA.ContentTypes.CONSUMABLE] < 5 or \
...
func determine_pack_pull(ExpansionID : DATA.ExpansionIDs) -> Dictionary:
	...
	var paired_rarities : Array[DATA.Rarities] = determine_pack_content_rarities(ExpansionID, pack_rarity, false)
	var content         : Array[Card]          = determine_pack_contents(ExpansionID, content_rarities, paired_rarities)
...
func determine_pack_contents(ExpansionID : DATA.ExpansionIDs, ContentRarities : Array[DATA.Rarities], PairedRarities : Array[DATA.Rarities]) -> Array[Card]:
	var cards: Array[Card] = []
	for i in ContentRarities.size():
		var card              : Card
		var rarity            :     = ContentRarities[i]
		var all_counts        : int = DATA.get_expansion_content_count(ExpansionID, rarity, DATA.ContentTypes.CRITTER)    \
									+ DATA.get_expansion_content_count(ExpansionID, rarity, DATA.ContentTypes.CONSUMABLE) \
									+ DATA.get_expansion_content_count(ExpansionID, rarity, DATA.ContentTypes.WEAPON)
		var idx               : int = floor(all_counts * RNG.random_value())
		for t in DATA.ContentTypes : 
			var count: int = DATA.get_expansion_content_count(ExpansionID, rarity, DATA.ContentTypes.get(t))
			if idx < count: card = DATA.get_expansion_content(ExpansionID, rarity, DATA.ContentTypes.get(t), idx); break;
			else: idx -= count
		 
		var paired_rarity     :     = PairedRarities[i]
		var paired_type_counts: int = DATA.get_expansion_content_count(ExpansionID, paired_rarity, card.Type)
		var paired_idx        : int = floor((paired_type_counts-(1 if paired_rarity == rarity else 0)) * RNG.random_value())
		paired_idx                 += int((paired_rarity == rarity) and (paired_idx >= idx) and paired_type_counts > 1)
		var paired_card       : Card= DATA.get_expansion_content(ExpansionID, paired_rarity, card.Type, paired_idx)
		
		assert(paired_type_counts <= 1 or not (card.Rarity == paired_card.Rarity and card.Type == paired_card.Type and card.ContentIndex == paired_card.ContentIndex))
		assert(card != null and paired_card!=null)
		cards.append(card)
		cards.append(paired_card)
	return cards


func DEBUG_roll_pack_odds(ExpansionID: DATA.ExpansionIDs, num_trials: int = 100) -> void:
...
func DEBUG_roll_booster_box_odds(ExpansionID: DATA.ExpansionIDs, num_trials: int = 25) -> void:
	print("beginning booster odds")
	var start = Time.get_unix_time_from_system()
	
	var total_counts: Array[int] = [0,0,0]
	for trial_num in range(num_trials):
		...
		while counts[DATA.ContentTypes.CRITTER]    < 5 or \
			...
			for i in range(pack_content.size()): 
				if not i%2: 
					counts[pack_content[i].Type]+=1
					total_counts[pack_content[i].Type]+=1
			
			for card in pack_content: card.free()
	
	var count_ev: float = 0.0
	for rarity in DATA.Rarities: count_ev += DATA.get_pack_content_count(ExpansionID, DATA.Rarities[rarity])*DATA.get_pack_rarity_odds(ExpansionID)[DATA.Rarities[rarity]]
	var total_cards: int = total_counts[0]+total_counts[1]+total_counts[2]
	LOGGER.log_msg("\nRESULTS FOR " + str(num_trials) + " \"BOOSTER BOXES\" OF " + DATA.ExpansionIDs.find_key(ExpansionID))
	LOGGER.log_msg("     CARDS per booster box: " + str(snappedf(float(total_cards)                               /float(num_trials), 0.001)))
	LOGGER.log_msg("   CRITTER per booster box: " + str(snappedf(float(total_counts[DATA.ContentTypes.CRITTER])   /float(num_trials), 0.001)))
	LOGGER.log_msg("CONSUMABLE per booster box: " + str(snappedf(float(total_counts[DATA.ContentTypes.CONSUMABLE])/float(num_trials), 0.001)))
	LOGGER.log_msg("    WEAPON per booster box: " + str(snappedf(float(total_counts[DATA.ContentTypes.WEAPON])    /float(num_trials), 0.001)))
	LOGGER.log_msg("               total cards: " + str(total_cards) + ".")
	LOGGER.log_msg("               thats worth: " + str(snappedf((float(total_cards)/float(num_trials)/count_ev), 0.001)) + " normal packs.")
	
	print("took ", Time.get_unix_time_from_system()-start)
		#var pull        : Dictionary    = determine_pack_pull(ExpansionID)
		#var pack_rarity : DATA.Rarities = pull["RARITY"]
		#var pack_content: Array[Card]   = pull["CONTENT"] 
#
